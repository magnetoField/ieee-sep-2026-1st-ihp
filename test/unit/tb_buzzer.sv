...
module tb_buzzer;
    ...
    wire tone_out, tone_busy;
    buzzer #(.BEEP_TICKS_MS(3),.TONE_HALF_TICKS(2)) tone(
    ...
    initial begin
        ...
        if (!tone_busy || tone_out) $fatal(1,"trigger priority/start wrong");
        pulse_scan(); if (tone_out) $fatal(1,"tone toggled early");
        pulse_scan(); if (!tone_out) $fatal(1,"tone half period wrong");
        pulse_ms(); if (!tone_busy) $fatal(1,"ended after first future tick");
        @(negedge clk); beep_event=1; @(posedge clk); @(negedge clk); beep_event=0;
        pulse_ms(); if (!tone_busy) $fatal(1,"ended after second future tick");
        @(negedge clk); beep_event=1; ms_tick=1; @(posedge clk); @(negedge clk); beep_event=0; ms_tick=0;
        if (tone_busy || tone_out) $fatal(1,"busy trigger extended/end wrong");
        beep_event=1; @(posedge clk); @(negedge clk); beep_event=0;
        if (!tone_busy) $fatal(1,"did not rearm");
        rst_n=0; #1; if(tone_out || tone_busy) $fatal(1,"reset unsafe");
        $display("PASS tb_buzzer passive timing and busy trigger policy"); $finish;
    end
endmodule
